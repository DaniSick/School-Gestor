<div class="flex h-screen">
    ...
    <div class="flex-1 p-6 overflow-y-auto">
        <div class="bg-gray-200 p-4 flex justify-between items-center">
            <h1 class="text-2xl font-bold">Balanza General</h1>
            ...
            <div class="overflow-x-auto">
                    ...
                    <table class="w-full border-collapse border border-gray-300">
                        ...
                        <tbody>
                            ...
                            @foreach ($cuentas as $cuenta)
                                ...
                                @endphp
                                <tr>
                                    <td class="border border-gray-300 px-4 py-2">{{ $cuenta->numero }}</td>
                                    <td class="border border-gray-300 px-4 py-2">{{ $cuenta->nombre }}</td>
                                    <td class="border border-gray-300 px-4 py-2 text-right">
                                        ${{ number_format($cuenta->total_cargo_periodo, 2) }}
                                    </td>
                                    <td class="border border-gray-300 px-4 py-2 text-right">
                                        ${{ number_format($cuenta->total_abono_periodo, 2) }}
                                    </td>
                                    <td class="border border-gray-300 px-4 py-2 text-right">
                            ...
                            <tr class="bg-gray-100 font-bold">
                                <td colspan="2" class="border border-gray-300 px-4 py-2 text-right">TOTALES:</td>
                                <td class="border border-gray-300 px-4 py-2 text-right">${{ number_format($totalCargo, 2) }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-right">${{ number_format($totalAbono, 2) }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-right">
                                    ${{ number_format($totalCargo - $totalAbono, 2) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                @else
                    <div class="text-center py-10">
                        ...
                        <h3 class="text-xl font-medium text-gray-500">No hay cuentas con movimientos en el período seleccionado</h3>
                        <p class="text-gray-500 mt-2">Intenta seleccionar un rango de fechas diferente o activa la opción "Mostrar cuentas sin movimientos".</p>
                    </div>
                @endif
            </div>
        </div>
